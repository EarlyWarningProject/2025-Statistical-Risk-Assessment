...
#setwd("/Users/nmbryce/Documents/R/SRA/2025-Statistical-Risk-Assessment/3.Modeling")
setwd("C:\\Users\\Vincent\\Dropbox\\EWP\\")
savenameaffix = "4Sep2025"
...
#getwd()
# Read in data ====
alldata = read.csv("2025-Statistical-Risk-Assessment/3.Modeling/prepared2024predictors-2025-09-03.csv")

# if working from rproj:
#alldata = read.csv("3.Modeling/prepared2024predictors-2025-09-03.csv")
...
traindata.full = alldata %>% filter(year<=2022) %>% 
...
# Repeat for 2023's forecasts
# Construct training and test data, pausing to look at missingness.
traindata.full.2023 = alldata %>% filter(year<=2021) %>% 
  select(all_of(c(predictornames.fullest, outcomenames, extravariables)))
nrow(traindata.full.2023)
table(traindata.full.2023$year)

#now remove missing to compare
traindata.full.2023 = traindata.full.2023 %>% filter(complete.cases(.))
nrow(traindata.full.2023)
table(traindata.full.2023$year)

# Test data:
testdata.full = alldata %>% filter(year==2024) %>% 
                      select(all_of(c(predictornames.fullest, extravariables)))

testdata.full.2023 = alldata %>% filter(year==2023) %>% 
  select(all_of(c(predictornames.fullest, extravariables)))
...
testdata.full.2023 = testdata.full.2023 %>% filter(complete.cases(.))
nrow(testdata.full.2023)
...
saveRDS(model.full.out$elastic.cv, 
        file = paste0("main_full_cv_glmnet_object_",savenameaffix,".rds"))
...
results.full = data.frame(ewp_name = testdata.full$ewp_name, 
                     risk_1or2.from2024 = model.full.out$risk) %>%
  mutate(risk_rank.from2024 = rank(desc(risk_1or2.from2024), na.last = "keep")) %>% 
  arrange(desc(risk_1or2.from2024))

# For last year (2023-based) predictions
model.full.2023.out = elogit.fit(ytrain=traindata.full.2023$mk_onset_1or2, 
                            Xtrain = traindata.full.2023,
                            Xtest = testdata.full.2023,
                            alpha = .5,
                            usepredictors = predictornames.fullest)

model.full.2023.out$coeffs

full.2023.coefs.df = data.frame(
  variable = rownames(model.full.2023.out$coeffs),
  coef.full.2023 = as.vector(model.full.2023.out$coeffs))

#Add odds ratio
full.2023.coefs.df = full.2023.coefs.df %>% 
  mutate(or.full.2023 = exp(coef.full.2023))

results.full.2023 = data.frame(ewp_name = testdata.full.2023$ewp_name, 
                          risk_1or2.from2023 = model.full.2023.out$risk) %>%
  mutate(risk_rank.from2023 = rank(desc(risk_1or2.from2023), na.last = "keep")) %>% 
  arrange(desc(risk_1or2.from2023))

#View(results.full.2023)
...
results.1yr = data.frame(ewp_name = testdata.full$ewp_name, 
                         risk.2025only.from2024 = model.1yr.out$risk) %>%
  mutate(riskrank.2025only.from2024 = rank(desc(risk.2025only.from2024), na.last = "keep")) 
...
bothforecastsandprior = bothforecasts %>% left_join(results.full.2023, by="ewp_name")
...
finalresults = finalresults %>% 
  rename(risk.2025and2026.from2024 = risk_1or2.from2024,
         rank.2025and2026.from2025 = risk_rank.from2024,
         risk.2024and2025.from2023 = risk_1or2.from2023, 
         rank.2024and2025.from2023 = risk_rank.from2023)

View(finalresults)

# Round as per Vincent's request
# 2025-11-15, this was messing me up, it should only be rounding the 
# forecasts, not everyting else
finalresults <- finalresults %>%
  mutate(risk.2025and2026.from2024 = signif(risk.2025and2026.from2024, 4),
         rank.2025and2026.from2025 = signif(rank.2025and2026.from2025, 4),
         risk.2024and2025.from2023 = signif(risk.2024and2025.from2023, 4),
         rank.2024and2025.from2023 = signif(rank.2024and2025.from2023, 4))

# Now add 2023-based forecasts for comparison in prior year
write.csv(finalresults, file=paste0("final_forecasts_data_",savenameaffix,".csv")) # was introducing rounding errors?
...
library(data.table)
fwrite(drop_out_df_sorted, 
       paste0("reg_model_dropouts_",savenameaffix, ".csv"))
...
merged_results = merged_results %>% arrange(desc(risk_1or2.from2024))
...
# 1987 commonly used.
lasttrainyear.series <- seq(1991, 2021, by=1)
...
catchplot <- ggplot(onsetforecasts, aes(x = maxrisk2)) +
  ...
  labs(title = "CDF of risk for countries with onsets after",
       x = "risk",
       y = "fraction caught") +
  theme_minimal() +
  scale_x_reverse(expand = expansion(mult = c(0.15, 0.15))) +  # extra padding
...
pdf(paste0("roc_curve",savenameaffix,".pdf"), width = 6, height = 6)
plot(roc_curve, main = "ROC Curve", col = "blue", lwd = 2)
dev.off()
