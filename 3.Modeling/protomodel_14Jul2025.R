...
#getwd()
# Read in data ====
alldata = read.csv("2025-Statistical-Risk-Assessment/3.Modeling/prepared2024predictors-2025-08-07.csv")
#alldata = read.csv("./3.Modeling/prepared2024predictors-2025-08-07.csv")
...
saveRDS(model.full.out$elastic.cv, file = "main_full_cv_glmnet_object_7Aug2025.rds")
...
library(data.table)
fwrite(drop_out_df_sorted, "reg_model_dropouts_7Aug.csv")
...
catchplot = ggplot(onsetforecasts, aes(x = maxrisk2)) +
  ...
  labs(title = "Full Model - Empirical CDF of Risk for Countries with onsets after",
       x = "risk",
       y = "fraction caught") +
  theme_minimal() +
  scale_x_reverse() +
